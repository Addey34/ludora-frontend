...
const games = [
  ...
  },
  {
    ...
    leaderboard: true,
    controls: [
      { keys: '<kbd>← →</kbd> or <kbd>A D</kbd>', action: 'Move your ship' },
      { keys: '<kbd>Space</kbd> / click', action: 'Shoot' },
  ...
  },
  {
    ...
    controls: [
      ...
      { keys: 'Goal', action: 'Claim more boxes than your opponent' },
    ],
  },
  {
    key: 'yahtzee',
    ...
    leaderboard: true,
    controls: [
      { keys: 'Roll button', action: 'Roll all non-held dice (up to 3 times per turn)' },
      { keys: 'Click a die', action: 'Hold it (keep between rolls)' },
      { keys: 'Click a category', action: 'Score the current dice in that category' },
      { keys: 'Goal', action: 'Maximise your total score across 13 categories' },
    ],
  },
  {
    key: 'binairo',
